...
import { addCartItem, clearCart, createCart, deleteCartItem, findCart, getCartId, updateCartItem } from "../apis/cart";
...
export function useClearCart() { const client = useQueryClient(); return useMutation({ mutationFn: () => clearCart(), onSuccess: () => client.invalidateQueries({ queryKey: ["cart"] }) }); }